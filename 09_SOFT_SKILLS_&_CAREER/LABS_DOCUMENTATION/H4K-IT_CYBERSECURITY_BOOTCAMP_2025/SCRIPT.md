...
- next ...**ScriptServe**: The website gaI started with basic uploads, but when they failed, I moved to discovery. I fuzzed the /preview endpoint using FFUF and eventually retrieved a file named `flag.txt` directly from the server. It taught me that even secure-looking upload systems can leak if endpoints aren’t protected properly.”
