...
- next ...**ScriptServe**: The website gave me a file upload form. I started with basic uploads, but when they failed, I switched gears to discovery. I used Gobuster to discover a `/preview` endpoint. and eventually retrieved a file named `flag.txt` directly from the server. It taught me that even secure-looking upload systems can leak if endpoints aren’t protected properly.”
