...
```shell-session
codenvibes@htb[/htb]$ curl -I https://www.inlanefreight.com
...
```shell-session
codenvibes@htb[/htb]$ curl https://www.inlanefreight.com -A 'Mozilla/5.0'
...
![[Pasted image 20250805230838.png]]
```shell
┌──(mopsy㉿APHP)-[~/HTB]
└─$ curl -X POST -d 'username=admin&password=admin' http://83.136.253.59:42306/
...
```shell
┌──(mopsy㉿APHP)-[~/HTB]
└─$ curl -X POST -d 'username=admin&password=admin' http://83.136.253.59:42306/ -i
HTTP/1.1 200 OK
Date: Tue, 05 Aug 2025 20:20:14 GMT
Server: Apache/2.4.41 (Ubuntu)
Set-Cookie: PHPSESSID=crv76vhr9i4goot78ami87ec9m; path=/
Expires: Thu, 19 Nov 1981 08:52:00 GMT
Cache-Control: no-store, no-cache, must-revalidate
Pragma: no-cache
Vary: Accept-Encoding
