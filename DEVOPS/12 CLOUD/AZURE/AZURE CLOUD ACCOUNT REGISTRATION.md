---
tags:
  - CYBER_SHUJAA_CNS
link:
description:
---
## Summary

| SECTION/TASK | FLAG |
| ------------ | ---- |
|              |      |

<div align="center">
<br>
<br>
※※※※※※※※※※※※※※※※※※※※※※※※
<br>
</div>
<!-- PAGE BREAK -->
<div style="page-break-after: always;"></div>

## Account Registration

Go to the link [https://azure.microsoft.com/en-us/free/](https://azure.microsoft.com/en-us/free/) to register a free Azure cloud account.

The free account will give you:  
•    Free 12 month access to the popular Azure services  
•    Access to over 55+ other services free  
•    Additional USD$ 200 worth of credit to use in first 30 days in addition to free use to cover any additional paid services

**Steps to follow:**

1. Click the **‘Start Free’** button.
2. Log in with your Microsoft account (Hotmail, Live, Outlook etc.) or GitHub account to link Azure to your GitHub account.
3. Fill in your Profile section details.
4. Provide your identity using a Phone number and verify using a text or call.
5. Provide your identity using a Bank Card. 
6. Complete the Agreement section to complete the registration.
7. Go to the Azure Portal [https://portal.azure.com/](https://portal.azure.com/) and verify your account is working.
8. Provide evidence **(upload as PDF)** showing your operational account capturing a screen shot of your free trial subscription working. Examples of screenshots are as follows:
<div align="center">
<br>
<br>
※※※※※※※※※※※※※※※※※※※※※※※※
<br>
</div>
<!-- PAGE BREAK -->
<div style="page-break-after: always;"></div>

## References

