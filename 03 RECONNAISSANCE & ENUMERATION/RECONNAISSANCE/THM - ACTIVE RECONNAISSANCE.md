#THM-JR_PENETRATION_TESTER #THMMOD-NETWORK_SECURITY
...
On Linux and macOS, the command to use is `traceroute 10.10.174.177`, and on MS Windows, it is `tracert 10.10.174.177`. `traceroute` tries to discover the routers across the path from your system to the target system.
...
However, the telnet client, with its simplicity, can be used for other purposes. Knowing that telnet client relies on the TCP protocol, you can use Telnet to connect to any service and grab its banner. Using `telnet 10.10.174.177 PORT`, you can connect to any service running on TCP and even exchange a few messages unless it uses encryption.
...
```shell-session
pentester@TryHackMe$ telnet 10.10.174.177 80
...
In the terminal shown above, we used netcat to connect to 10.10.174.177 port 80 using `nc 10.10.174.177 80`. Next, we issued a get for the default page using `GET / HTTP/1.1`; we are specifying to the target server that our client supports HTTP version 1.1. Finally, we need to give a name to our host, so we added on a new line, `host: netcat`; you can name your host anything as this has no impact on this exercise.
...
On the _client_-side, you would issue `nc 10.10.174.177 PORT_NUMBER`. Here is an example of using `nc` to echo. After you successfully establish a connection to the server, whatever you type on the client-side will be echoed on the server-side and vice versa.

Consider the following example. On the server-side, we will listen on port 1234. We can achieve this with the command `nc -vnlp 1234` (same as `nc -lvnp 1234`). In our case, the listening server has the IP address `10.10.174.177`, so we can connect to it from the client-side by executing `nc 10.10.174.177 1234`. This setup would echo whatever you type on one side to the other side of the TCP tunnel. You can find a recording of the process below. Note that the listening server is on the left side of the screen.
<div>
<br>
<br>
</div>
...
|Command|Example|
|---|---|
|ping|`ping -c 10 10.10.92.224` on Linux or macOS|
|ping|`ping -n 10 10.10.92.224` on MS Windows|
|traceroute|`traceroute 10.10.92.224` on Linux or macOS|
|tracert|`tracert 10.10.92.224` on MS Windows|
|telnet|`telnet 10.10.92.224 PORT_NUMBER`|
|netcat as client|`nc 10.10.92.224 PORT_NUMBER`|
